...
import { FiEdit2, FiTrash2, FiPlus, FiCheckCircle, FiXCircle, FiLock, FiX, FiUsers } from "react-icons/fi";
...
const RoleManagement: React.FC = () => {
  ...
  if (loading) return <div className="flex justify-center items-center h-64 text-gray-500">Loading data...</div>;
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {roles.map((role) => (
          <div key={role.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition p-5 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                    {role.name}
                    {role.isSystem && <FiLock className="text-gray-400 w-4 h-4" title="System Role" />}
                  </h3>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {role.permissions?.slice(0, 3).map((p) => (
                    <span key={p.id} className="px-2 py-1 text-xs border border-gray-200 rounded-md text-gray-600 bg-gray-50">
                      {p.name}
                    </span>
                  ))}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-2">
              ...
              <span className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${
                ...
                {role.isActive ? <><FiCheckCircle size={12}/> Active</> : <><FiXCircle size={12}/> Inactive</>}
              </span>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl relative z-10 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider mb-3">Permissions</h3>
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  {Object.entries(groupedPermissions).map(([category, perms]) => (
                    <div key={category} className="border-b last:border-0 border-gray-100">
                      <div className="bg-gray-50 px-4 py-2 text-xs font-bold text-gray-600 uppercase">
                        {category}
                      </div>
                      <div className="p-2 grid grid-cols-1 sm:grid-cols-2 gap-2 bg-white">
                        {perms.map((permission) => (
                          <label 
                            key={permission.id} 
                            className="flex items-start gap-3 p-2 rounded hover:bg-blue-50 cursor-pointer transition select-none group"
                          >
                            <input
                              type="checkbox"
                            ...
                            />
                            <div>
                              <p className="text-sm font-medium text-gray-700 group-hover:text-blue-700">{permission.name}</p>
                ...
                <p className="text-right text-xs text-gray-500 mt-2">Selected: {selectedPermissions.length}</p>
              </div>
            </div>
